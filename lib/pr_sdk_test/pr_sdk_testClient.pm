package pr_sdk_test::pr_sdk_testClient;
...
sub new
{
    ...
    my $self = {
	client => pr_sdk_test::pr_sdk_testClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
<pre>
$params is a pr_sdk_test.FilterContigsParams
$output is a pr_sdk_test.FilterContigsResults
FilterContigsParams is a reference to a hash where the following keys are defined:
	assembly_input_ref has a value which is a pr_sdk_test.assembly_ref
...
FilterContigsResults is a reference to a hash where the following keys are defined:
	...
	assembly_output has a value which is a pr_sdk_test.assembly_ref
...
# Authentication: required
    ...
    my $result = $self->{client}->call($url, $self->{headers}, {
	    method => "pr_sdk_test.filter_contigs",
	    params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for pr_sdk_test::pr_sdk_testClient\n";
    }
    if ($sMajor == 0) {
        warn "pr_sdk_test::pr_sdk_testClient version is $svr_version. API subject to change.\n";
    }
}
...
package pr_sdk_test::pr_sdk_testClient::RpcClient;
